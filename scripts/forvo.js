...
const { getNotes, ensure } = require('../src/ankihelpers')
...
const forvoMan = async () => {
  ...
  const next = async () => {
    ...
    const extraRaw = note.fields['Gender, Personal Connection, Extra Info (Back side)'].value
    console.log('extraRaw: ', extraRaw)
    const extra = h2p(extraRaw.replace(/<div>/g, '\n').replace(/<\/div>/g, '\n') + ' ')
